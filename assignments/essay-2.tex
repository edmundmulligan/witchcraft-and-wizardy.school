...
the source code (available on the main branch at 
...
The third principle required meaningful names and a consistent naming convention
was used throughout.
...
clamp~() is defined using a viewport width unit (vw), then the text will not scale if 
...
middle value of clamp~() from a vw unit and a rem unit, so that zooming 
...
particularly wanted to try was popover to avoid having to use JavaScript alert~(). 
...
Table~\ref{tab:bugs} in Appendix~\ref{sec:bugs} lists some known issues and areas 
