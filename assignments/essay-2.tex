...
\posttitle{\par\end{center}\begin{center}\large\url{https://witches-and-wizards.edmundmulligan.name}\end{center}\vskip 0.5em}
...
the source code, which should be read alongside this essay (available on the main branch at 
\url{https://github.com/Birkbeck2/puffin-web-development-website-edmundmulligan}), so the essay 
...
it was well suited to this small project with a single developer fulfilling all team roles. The
...
The second principle influenced the use of 
...
and camelCase for JavaScript variables and functions (although, admittedly, there is currently 
only one JavaScript function).

A fourth principle, which is not referenced as I worked it out 
the hard way, is Keep the CSS as simple as possible and make use of the power of cascading. I am 
sure this is something experienced web developers know and if I were to redo this project I 
would put much more effort into writing simple CSS first time rather than trying to refactor 
complex CSS later. The current codebase is still not optimal in this regard and with more time 
could be improved.
...
\section{A Responsive and Accessible Website}
...
in this area has been highly influenced by three YouTube channels, those of Kevin Powell 
\citep{powell_youtube}, Daily Css \citep{dailycss_youtube} and 
Coding2GO \citep{coding2go_youtube}, all of which produce 
...
experience (which I lack) and judgement (which I possess in abundance), 
...
this is still a work in progress and further refinements could be made in the 
future. The process was made a lot easier when I discovered, at a late stage, 
an online tool to help calculate fluid typography values \citep{bece2025}. 
...
My conclusion for this project was to use flexbox in narrower viewports 
(mobile layout) as this could make vertical stacking of elements easier with 
flex-direction: column, and to prefer flexbox with flex-direction: row in wider 
...
rather than absolute units. Thus, I used rem units for specifying element sizes. 
I avoided using px units except in media queries where absolute measurements 
...
the choices passed the contrast tests in the accessibility tools. The putple and 
cyan colours were choses as they (approximately) match the Embodied Mind logo used 
in the footer; the yellow and green were more arbitary.

Testing without a mouse and with a screen reader ensured that that the~::focus 
...
could potentially be caught by one of the other tools, but it did introduce a problem of
...
usage as it may have accessibility implications.
...
from this phase as it will be developed in phase 2 using JavaScript. Table \ref{tab:bugs}
lists some known issues and areas for improvement that could not be addressed within the
...
\toprule
1 & Reactive code does not work well at extreme viewport sizes & Text overflows boxes or content does not fill screen properly. & Warning displayed when viewport is too small.\hfill \\
\midrule
2 & Font sizes are not optimal\hfill & Text is too small or too large on some devices. & Further refinement of clamp() values needed.\hfill \\
\midrule
3 & Images need editing. The artwork provided by Rachel Mulligan is in JPEG format with a white background. & The picture gallery looks unprofessional with white boxes around images. & Images need to be edited to have transparent backgrounds (PNG or SVG format). \\
\bottomrule
\end{tabular}
